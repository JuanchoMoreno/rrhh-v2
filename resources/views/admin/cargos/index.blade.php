            ...
            <div class="card card-outline card-purple shadow p-3 mb-5 bg-body rounded">
                <div class="card-header">
                    ...
                    <div class="card-tools">
                        <a href="{{ url('/admin/cargos/create') }}" class="btn btn-primary btn-sm"><i class="fas fa-plus"></i> Nuevo</a>
                    </div>
                </div>
                <div class="card-body">
                                ...
                                <tr>
                                    ...
                                    <td style="text-align: center">
                                        <div class="btn-group" role="group" aria-label="Basic mixed styles example">
                                            <a href="{{ url('/admin/cargos', $cargo->id) }}" class="btn btn-info btn-sm fas fa-eye"></a>
                                            <a href="{{ url('/admin/cargos/' . $cargo->id . '/edit') }}" class="btn btn-warning btn-sm fas fa-edit"></a>
                                            @if ($cargo->id != 1)
                                                <form action="{{ url('/admin/cargos', $cargo->id) }}" class="btn-group" method="post" onclick="pregunta{{ $cargo->id }}(event)" id="formDelete{{ $cargo->id }}">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn btn-danger btn-sm fas fa-trash-alt" role="group"></button>
                                                </form>
                                            @else
                                                <button type="button" class="btn btn-danger btn-sm fas fa-trash-alt btn-group" role="group" disabled></button>
                                            @endif
                                            <script>
                                                function pregunta{{ $cargo->id }}(event) {
